...
from .copy_paste import random_add_patches
...
class CenterCrop(object):
    def __init__(self, w_ratio=0.12, h_down=0.08):
    ...
    def __call__(self, results):
        ...
        if np.random.rand() > 0.5:
            w_ = np.random.rand() * self.w_ratio
            h_down_ = np.random.rand() * self.h_down

            left = int(w_ * w)
            right = w - left
            down = int((1 - h_down_) * h)

            results['img'] = results['img'][0:down, left:right]
            bboxes = results['gt_bboxes']

            bboxes[:, 0] = np.maximum(0, bboxes[:, 0] - left)
            bboxes[:, 2] = np.minimum(right, bboxes[:, 2] - left)
            bboxes[:, 3] = np.minimum(down, bboxes[:, 3])
...
@PIPELINES.register_module
class CenterCrop_pingshen(object):
    def __init__(self, w_ratio=0.2, h_up=0.1, h_down=0.1):
        self.w_ratio = w_ratio
        self.h_up = h_up
        self.h_down = h_down

    def __call__(self, results):
        mode = results['filename'].split('/')[-1].count('_')
        if mode == 2:
            return results
        h, w, _ = results['img_shape']
        ...
        if np.random.rand() > 0.5:
            w_ = np.random.rand() * self.w_ratio
            h_up_ = np.random.rand() * self.h_up
            h_down_ = np.random.rand() * self.h_down
            ...
            results['img'] = results['img'][up:down, left:]
            bboxes = results['gt_bboxes']

            bboxes[:, 0] = np.maximum(0, bboxes[:, 0] - left)
            bboxes[:, 1] = np.maximum(0, bboxes[:, 1] - up)
            bboxes[:, 2] = bboxes[:, 2] - left
            bboxes[:, 3] = bboxes[:, 3] - up
            results['gt_bboxes'] = bboxes
...
@PIPELINES.register_module
class Aug_cat12(object):
    def __init__(self):
        pass

    def flip_bbox(self, roi):
        roi = roi[:, ::-1, :]
        return roi

    def __call__(self, results):
        if np.all(results['gt_labels'] == 2):  # category 12
            img = results['img']
            ...
            area = w * h
            bboxes_small = bboxes[area < 5000] #.astype(int)

            if bboxes_small.shape[0] == 0:
                return results
            # bboxes = [list(map(int, i)) for i in results['gt_bboxes'].tolist()]

            for box in bboxes_small:
                box = list(map(int, box))
                paste_number = random.randint(2, 4)
                roi = img[box[1]:box[3], box[0]:box[2]]
                new_bboxes = random_add_patches(box, bboxes, img.shape,
                                                paste_number=paste_number,
                                                iou_thresh=0.2)

                count = 0
                for new_bbox in new_bboxes:
                    count += 1
                    bbox_left, bbox_top, bbox_right, bbox_bottom = new_bbox[0], new_bbox[1], new_bbox[2], new_bbox[3]
                    height, width, channels = roi.shape
                    center = (int(width / 2), int(height / 2))
                    mask = 255 * np.ones(roi.shape, roi.dtype)

                    if count > 1:
                        roi = self.flip_bbox(roi)

                    try:
                        img[bbox_top:bbox_bottom, bbox_left:bbox_right] = cv2.seamlessClone(roi,
                                                                                              img[bbox_top:bbox_bottom,
                    ...
                    bboxes_added.append(new_bbox)
            results['gt_bboxes'] = np.array(results['gt_bboxes'].tolist() + bboxes_added).astype(np.float32)
            results['gt_labels'] = np.array(results['gt_labels'].tolist() + [2] * len(bboxes_added))
            results['img'] = img
...
@PIPELINES.register_module
class RandomFlip_custom(object):
    """Flip the image & bbox & mask.
