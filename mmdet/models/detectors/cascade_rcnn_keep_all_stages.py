...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def nms_bboxes(self, img_meta, rois, cls_score, bbox_pred, rcnn_test_cfg, ms_bbox_result=None):
        ...
        if ms_bbox_result is not None:
            ...
            merged_bboxes, merged_scores = merge_aug_bboxes(
                aug_bboxes, aug_scores, [img_meta] * len(aug_bboxes), rcnn_test_cfg)
            det_bboxes, det_labels = multiclass_nms(merged_bboxes, merged_scores,
                                                    rcnn_test_cfg.score_thr,
                                                    rcnn_test_cfg.nms,
                                                    rcnn_test_cfg.max_per_img)
    ...
    def simple_test_bboxes_custom(self, x,
        ...
        cls_score = sum(ms_scores) / self.num_stages
        if not nms:
            return rois, cls_score, bbox_pred, [aug_bboxes, aug_scores]
    ...
    def forward_bbox_(self, x, img_meta, proposal_list):
        rois, cls, bbox, ms_bbox_result = self.simple_test_bboxes_custom(
                x, img_meta, proposal_list, self.test_cfg.rcnn, nms=False)
        return rois, cls, bbox, ms_bbox_result

    def forward_nms_(self, img_meta, rois, cls, bbox, ms_bbox_result):
        det_bboxes, det_labels = self.nms_bboxes(img_meta, rois, cls, bbox, self.test_cfg.rcnn, ms_bbox_result)
