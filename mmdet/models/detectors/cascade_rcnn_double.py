...
class CascadeRCNN_DoubleHead(BaseDetector, RPNTestMixin):
    ...
    def __init__(self,
        ...
        super(CascadeRCNN_DoubleHead, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(CascadeRCNN_DoubleHead, self).init_weights(pretrained)
    ...
    def forward_dummy(self, img):
        ...
        if self.with_bbox:
            for i in range(self.num_stages):
                bbox_cls_feats = self.bbox_roi_extractor[i](
                    x[:self.bbox_roi_extractor[i].num_inputs], rois)
                bbox_reg_feats = self.bbox_roi_extractor[i](
                    x[:self.bbox_roi_extractor[i].num_inputs],
                    rois,
                    roi_scale_factor=self.reg_roi_scale_factor)
                if self.with_shared_head:
                    bbox_cls_feats = self.shared_head(bbox_cls_feats)
                    bbox_reg_feats = self.shared_head(bbox_reg_feats)
                cls_score, bbox_pred = self.bbox_head[i](bbox_cls_feats, bbox_reg_feats)
    ...
    def show_result(self, data, result, **kwargs):
        ...
        super(CascadeRCNN_DoubleHead, self).show_result(data, result, **kwargs)
    ...
    def forward_roi_(self, x, img_meta, proposal_list):
        det_bboxes, det_labels = self.simple_test_bboxes_custom(
